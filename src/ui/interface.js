...
export class UI {
  ...
  constructor(game) {
    ...
    this.glowInterval = setInterval(() => {
      const opa = 0.5 + Math.floor((1 + Math.sin(Math.floor(new Date() * Math.PI * 0.20) / 100)) / 4 * 100) / 100;
  ...
  showCreature(creatureType, player) {
    ...
    const stats = game.retreiveCreatureStats(creatureType);
    ...
    if ($j.inArray(creatureType, game.players[player].availableCreatures) > 0 || creatureType == '--') {
      ...
      } else if (activeCreature.player.id === player && activeCreature.type === '--' && activeCreature.abilities[3].used === true) {
  ...
  gridSelectUp() {
    ...
    if (game.realms.indexOf(b[0]) - 1 > -1) {
      const r = game.realms[game.realms.indexOf(b[0]) - 1];
  ...
  gridSelectLeft() {
    const b = (this.selectedCreature == '--') ? 'A0' : this.selectedCreature;
  ...
  gridSelectRight() {
    const b = (this.selectedCreature == '--') ? 'A8' : this.selectedCreature;
  ...
  showStatModifiers(stat) {
    if (this.selectedCreatureObj instanceof Creature) {
      const buffDebuff = this.selectedCreatureObj.getBuffDebuff(stat);
  ...
  updateTimer() {
    ...
    if (game.timePool >= 0) {
      ...
      // Time Bar
      const poolRatio = (game.activeCreature.player.totalTimePool - (date - game.activeCreature.player.startTime)) / 1000 / game.timePool;
  ...
  updateQueueDisplay(excludeActiveCreature) {
    ...
    const queueAnimSpeed = this.queueAnimSpeed;
    ...
    const deleteVignette = (vignette) => {
      ...
      } else if ($j(vignette).hasClass('active')) {
        $j(vignette).attr('verified', -1).transition({
          x: -100,
          queue: false,
        }, queueAnimSpeed, transition, () => {
    ...
    const updatePos = () => {
      $vignettes.each((pos, vignette) => {
        ...
        const offset = (index - (!!index)) * 80 + (!!index) * 100;
        $j(vignette).css({
          'z-index': 0 - index,
        }).transition({
          x: offset,
      ...
      if (typeof completeQueue[i] === 'string') {
        ...
        if ($vignettes[i] === undefined) {
          ...
          while (true) {
            const v = $vignettes[i];
            const $v = $j(v);
            const vid = parseInt($v.attr('creatureid'));
    ...
    this.$queue.find('.vignette').not('.roundmarker').unbind('mousedown').unbind('mouseover')
      .unbind('mouseleave')
      .bind('mouseover', (e) => {
        if (game.freezedInput) {
          return;
  ...
  bouncexrayQueue(creaID) {
    this.xrayQueue(creaID);
    if (creaID > 0) { var $queueItem = this.$queue.find(`.vignette[creatureid="${creaID}"]:first`); }
    if ($queueItem.length > 0) {
      $queueItem.stop();
      $queueItem.animate({ top: '+=30px' }, 200, '', function () {
  ...
  updateFatigue() {
    ...
    game.creatures.forEach((creature) => {
      if (creature instanceof Creature) {
        const textElement = $j(`#queuewrapper .vignette[creatureid="${creature.id}"]`).children('.stats');
