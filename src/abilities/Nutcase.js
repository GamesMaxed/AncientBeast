  ...
  G.abilities[40] = [
    ...
    //  First Ability: Tentacle Bush
    {
      ...
      activate(damage) {
        ...
        const counterDamage = new Damage(
    ...
    //  Second Ability: Hammer Time
    {
      ...
      activate(targetOrChoice, args) {
        ...
        if (!this.isUpgraded()) {
          ...
          const yCoords = targetOrChoice.map(hex => hex.y);
          ...
          targetOrChoice.sort((a, b) => (yAscending ? a.y - b.y : b.y - a.y));
      ...
      _activateOnTarget(target) {
        ...
        const effect = new Effect(
          ...
          {
            effectFn(effect) {
              effect.target.takeDamage(new Damage(effect.owner, {
    ...
    //   Third Ability: War Horn
    {
      ...
      query() {
        ...
        if (!this.isUpgraded()) {
          ...
          for (let i = 0; i < o.choices.length; i++) {
            var j;
            const direction = o.choices[i][0].direction;
            ...
            // Exclude creature
            ability.creature.hexagons.forEach((hex) => {
      ...
      _pushTarget(target, pushPath, args) {
        const ability = this;
      ...
      _pushOneHex(target, hex, targetHex) {
        const opts = {
          overrideSpeed: 100,
          ignorePath: true,
          ignoreMovementPoint: true,
    ...
    //  Third Ability: Fishing Hook
    {
      ...
      require() {
        ...
        if (!this.atLeastOneTarget(this.creature.getHexMap(matrices.inlinefrontnback2hex), {
          team: this._targetTeam,
          optTest(creature) {
      ...
      activate(target, args) {
        ...
        const trgIsInfront = (G.grid.getHexMap(crea.x - inlinefront2hex.origin[0], crea.y - inlinefront2hex.origin[1], 0, false, inlinefront2hex)[0].creature == target);
